...
构建一个名为 `safe-claude-sbx` 的 macOS CLI。它读取 YAML 配置，执行 preflight 检查，只有在默认路由走 `utunX`、宿主机和 sandbox 出口 IP 都等于配置值、sandbox 内没有显式代理环境变量、workspace mount 安全时，才启动 Docker Sandbox / `sbx` 中的 Claude Code。

启动后，CLI 使用 `route -n monitor` 监听 macOS 路由变化。每次事件触发后重新检查默认路由、TUN 接口存在性和 sandbox 出口 IP。一旦任一检查失败，CLI 立即停止 sandbox 并执行统一 cleanup。
...
11. 作为维护者，我想把 macOS route 检查、egress 检查、mount policy 和 backend 控制分开，这样后续可以扩展普通 Docker 或 Apple container backend。
12. 作为维护者，我想有清晰错误码和日志，这样用户能定位是 TUN、出口 IP、`sbx`、Docker Sandbox 还是挂载策略失败。

## 实现决策

- 第一阶段只支持 macOS 和 Docker Sandbox / `sbx` backend。
- CLI 是唯一用户入口，提供正常运行命令和 `doctor` 检查命令。
- 配置使用 YAML，核心字段包括期望出口 IP、路由检查目标、IP 检查 URL、sandbox 名称、workspace mount、backend、timezone、locale、禁止代理环境变量和禁止挂载路径。
- preflight 必须 fail closed。任何网络、backend、egress、env var 或 mount policy 检查失败都不得启动主 sandbox。
- macOS 默认路由检查使用 `route get <route_check_target>`，并要求 interface 为 `utunX`。
- 启动时记录 `startup_tun_interface`，运行期间要求默认路由继续使用该接口。
- 运行期 route watcher 使用 `route -n monitor`，事件触发后重新检查路由和 sandbox egress。第一版默认不使用 5 到 10 秒低频兜底轮询。
- backend policy 和 platform policy 分离。Docker Sandbox / `sbx` 是 adapter，不承载核心安全策略。
...
- 项目文档必须明确安全边界：该工具不保证远端服务无法识别网络类型，也不替代账号、权限、付款或组织合规管理。

## 测试决策

- 最高价值测试接缝是 launcher 行为：给定配置、route/backend/egress/mount 的可观察结果，验证 CLI 是否启动、拒绝或 cleanup。
- 单元测试应覆盖配置校验、route 输出解析、mount policy、egress IP 比较、cleanup 幂等行为。
...
- 自定义 Claude Code 镜像或模板的完整维护。
- 账号、OAuth、API key、组织权限、付款状态或 Claude Code 认证流程管理。
- 绕过平台规则、服务端风控、账号限制、付款限制或网络识别。
