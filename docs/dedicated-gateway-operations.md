...
- attaching sandbox-local Herdr only after startup validation;
- revoking launcher-owned daemon state before main cleanup.
...
For a future explicitly supported backend, `doctor` and `safe-herdr` share this
flow:
...
4. `doctor` immediately calls `Revoke`. `safe-herdr` prepares and attaches Herdr
   only after validation, then enters the existing Watchdog Supervisor.
...
6. Every exit path cancels the active runtime check, calls `Revoke`, and only
   then cleans up main according to ownership and cleanup policy.
...
6. Run the guarded Herdr entrypoint only after doctor succeeds:

   ```bash
   safe-herdr --config config.yaml
   ```

Plain `safe-claude-sbx --config config.yaml` does not currently accept dedicated
mode. Dedicated startup uses `safe-herdr`; direct Claude support requires its own
...
| Gateway or controller loss | Runtime check fails closed, then revokes before main cleanup | Repair the external gateway, verify loopback controller health, and start a new guarded session |
...
| Revoke failure | Report `sandboxd lease revoke invalid`; main cleanup is still attempted | Treat daemon state as unknown and follow the explicit host-inherited recovery below |
| Main cleanup failure | Report cleanup failure after revoke attempt | Verify main state by name; stop it if policy requires, but do not remove a preexisting main |
...
1. Stop the launcher session and wait for its revoke/cleanup result. Do not run
   `sbx reset`.
2. Record the configured main sandbox's pre-recovery existence and state. Do
   not delete it.
3. If revoke succeeded, keep the restored normal sandboxd and continue to step
   5. If revoke failed, treat daemon state as unknown and stop it:
...
doctor, `safe-herdr`, runtime watchdog failures, HTTP/HTTPS no-fallback, generic
TCP, DNS, private Docker Engine pulls, revoke ordering, and cleanup.
...
- preexisting main existence and state are unchanged unless the configured
  cleanup policy explicitly stopped it;
