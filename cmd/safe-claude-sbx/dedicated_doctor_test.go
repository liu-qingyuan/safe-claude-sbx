...
func TestDoctorValidatesSupportedDedicatedGatewayAndCleansUpInOrder(t *testing.T) {
	...
	fakeSBX := writeDedicatedSupportedFakeSBX(t, logPath, "203.0.113.10")
	cmd := exec.Command("go", "run", "-tags=safe_claude_sbx_e2e", ".", "doctor", "--config", configPath)
	cmd.Dir = "."
...
func TestDoctorSupportedDedicatedGatewayFailsClosed(t *testing.T) {
	tests := []struct {
		name                string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			fakeSBX := writeDedicatedSupportedFakeSBXWithOptions(t, logPath, dedicatedSupportedFakeSBXOptions{
				EgressIP:            egressIP,
				UnrelatedSandbox:    tt.unrelatedRunning,
				ControllerReachable: tt.controllerReachable,
			})
			cmd := exec.Command("go", "run", "-tags=safe_claude_sbx_e2e", ".", "doctor", "--config", configPath)
			cmd.Dir = "."
...
func TestDoctorSupportedDedicatedGatewayPreservesExistingMainAfterInspectionFailure(t *testing.T) {
	...
	configPath := writeTestConfig(t, dedicatedDoctorConfig(controller.URL, "203.0.113.10"))
	logPath := filepath.Join(t.TempDir(), "sbx.log")
	fakeSBX := writeDedicatedSupportedExistingMainFakeSBX(t, logPath, inspectionSecret)
	cmd := exec.Command("go", "run", "-tags=safe_claude_sbx_e2e", ".", "doctor", "--config", configPath)
	cmd.Dir = "."
	cmd.Env = append(os.Environ(), "PATH="+fakeSBX+string(os.PathListSeparator)+os.Getenv("PATH"))
...
func TestDoctorSupportedDedicatedGatewayRuntimeFailures(t *testing.T) {
	tests := []struct {
		name              string
...
func writeDedicatedSupportedFakeSBX(t *testing.T, logPath, egressIP string) string {
	return writeDedicatedSupportedFakeSBXWithOptions(t, logPath, dedicatedSupportedFakeSBXOptions{EgressIP: egressIP})
}

type dedicatedSupportedFakeSBXOptions struct {
	EgressIP              string
...
func writeDedicatedSupportedFakeSBXWithOptions(t *testing.T, logPath string, opts dedicatedSupportedFakeSBXOptions) string {
	t.Helper()
	dir := t.TempDir()
...
func writeDedicatedSupportedExistingMainFakeSBX(t *testing.T, logPath, inspectionSecret string) string {
	t.Helper()
	dir := t.TempDir()
	readyPath := filepath.Join(dir, "daemon-ready")
	stopPath := filepath.Join(dir, "daemon-stop")
